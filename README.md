...
Стартует с **пустого проекта** — задачи добавляете вручную, через чат или импорт Excel.
...
После входа — пустой план «Новый проект». Пример заполненного плана: [examples/plan_vax_b_demo.xlsx](examples/plan_vax_b_demo.xlsx).

### Проверка за 3 минуты

1. Войти как `pm` → пустой Gantt.
...
- `make seed` — пользователь `pm` + пустой план
- `make test` — pytest
