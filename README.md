...
Демо-ролик: [docs/demo.gif](docs/demo.gif) (~31 с) — логин → сид-Gantt VAX-B → импорт Excel → правка через чат → отмена → экспорт → журнал. Чеклист: [docs/DEMO.md](docs/DEMO.md).
...
| Логин | Пароль | Роль |
|-------|--------|------|
| `pm` | `pm12345` | editor (полный доступ) |
| `viewer` | `viewer123` | viewer (только просмотр / экспорт) |

### Проверка за 3 минуты (wow-path)

1. Открыть https://bio.2alexs.ru → войти как `pm` → виден сид-Gantt вакцины **VAX-B** (discovery → коммерческий выпуск, РФ).
2. **Импорт Excel** → `examples/plan_vax_b_demo.xlsx` (или прикрепить файл в чат и написать «импортируй»).
...
- Excel: колонки задания (`задача`, `описание`, `исполнитель`, `длительность`, `предшественники`) — достаточно для импорта; `код`/`родитель` опциональны (автокод `T1…Tn`).
- RBAC: `viewer` — только просмотр/экспорт; `pm` — editor.
