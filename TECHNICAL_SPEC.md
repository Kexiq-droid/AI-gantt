...
2. Загрузить `examples/plan_biokad_demo.xlsx` (чуть отличный от сида план) → диаграмма обновилась.
3. В чат:  
...
| `длительность` | int | Рабочие дни (> 0 для листьев; у фаз — вычисляемо или сумма) |
| `предшественники` | string | Коды через запятую (`T1,T3`); тип связи MVP: FS |
| `родитель` | string | Код родителя; пусто = корень |

Единица времени MVP: **календарные дни как рабочие** от `plan.start_date` (без праздников РФ — долг Roadmap).  
Расчёт `start_date` листа: max(конец предшественников), иначе start плана / после родителя по правилам планировщика (документировать в README одну простую формулу).

### 5.2 Сид-сценарий

**Название плана:** Вывод препарата-кандидата BIO-X на клинические исследования (фаза I)

Фазы (примерная структура):

1. Аналитика и целеполагание  
2. Доклинические исследования  
3. CMC / производство  
4. Регуляторика  
5. Клиника I (подготовка)

Внутри — задачи и 1 уровень подзадач. Исполнители вымышленные (Иванов, Петрова, …). Объём: ~25–40 строк — достаточно для демо, не перегруз.

### 5.3 Sample-файл

`examples/plan_biokad_demo.xlsx` — валидный импорт, немного отличается от сида (чтобы шаг 2 demo script был заметен).
...
| G3 | «Добавь задачу "Резервный анализ образцов" в доклинику на 5 дней после T2.3» | create + predecessor |
